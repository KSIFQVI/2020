...
def test(policy, market, optimizer, scheduler, test_begin):
    ...
    for _ in count(1):
        ...
        stop_date = market.last_date() - relativedelta(months=13) + relativedelta(day=31)
...
def load_model(path, eval_flag, device):
    #Load Model
    policy = torch.load(path, map_location=torch.device(device))
...
def select_portfolio(data_path, model_path, invest_date, FACTORS, device):
    ...
    policy = load_model(model_path, eval_flag=True, device = device_type)
